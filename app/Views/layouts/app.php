...
<body class="<?= $theme === 'dark' ? 'theme-dark' : 'theme-light' ?>">
    <div class="app-shell">
        <aside class="sidebar" id="sidebar">
            <button class="sidebar-collapse-button" id="desktop-sidebar-toggle" type="button" aria-label="Recolher menu"
                title="Recolher menu">
            ...
            <nav class="nav-menu">
                <a class="nav-item <?= ($active ?? '') === 'dashboard' ? 'active' : '' ?>" data-tooltip="Dashboard"
                    href="<?= url('dashboard') ?>">
                    <i data-lucide="house" class="nav-icon"></i><span>Dashboard</span>
                </a>

                <a class="nav-item <?= ($active ?? '') === 'point' ? 'active' : '' ?>" data-tooltip="Meu Ponto"
                    href="<?= url('dashboard') ?>#meu-ponto">
                    <i data-lucide="clock-3" class="nav-icon"></i><span>Meu Ponto</span>
                </a>

                <a class="nav-item <?= ($active ?? '') === 'history' ? 'active' : '' ?>" data-tooltip="Histórico"
                    href="<?= url('history') ?>">
                    <i data-lucide="notebook-tabs" class="nav-icon"></i><span>Histórico</span>
                </a>

                <a class="nav-item <?= ($active ?? '') === 'calendar' ? 'active' : '' ?>" data-tooltip="Calendário"
                    href="<?= url('calendar') ?>">
                    <i data-lucide="calendar-days" class="nav-icon"></i><span>Calendário</span>
                </a>

                <a class="nav-item <?= ($active ?? '') === 'absences' ? 'active' : '' ?>"
                    data-tooltip="Ausências e Justificativas" href="<?= url('absences') ?>">
                    <i data-lucide="calendar-off" class="nav-icon"></i><span>Ausências e Justificativas</span>
                </a>

                <a class="nav-item <?= ($active ?? '') === 'reports' ? 'active' : '' ?>" data-tooltip="Relatórios"
                    href="<?= url('reports') ?>">
                    <i data-lucide="chart-no-axes-column-increasing" class="nav-icon"></i><span>Relatórios</span>
                </a>


                <a class="nav-item <?= ($active ?? '') === 'profile' ? 'active' : '' ?>" data-tooltip="Perfil"
                    href="<?= url('profile') ?>">
                    <i data-lucide="user-round" class="nav-icon"></i><span>Perfil</span>
                </a>

                <a class="nav-item <?= ($active ?? '') === 'settings' ? 'active' : '' ?>" data-tooltip="Configurações"
                    href="<?= url('settings') ?>">
            ...
            <div class="sidebar-footer">
                <a href="<?= url('profile') ?>" class="mini-profile" data-tooltip="Perfil">
                    <?php if ($avatarPath): ?>
                        <img src="<?= config('app.url') . '/' . e($avatarPath) ?>" alt="Foto de perfil"
            ...
            <header class="topbar">
                ...
                <form class="search-box global-search-form" method="GET" action="<?= config('app.url') ?>">
                    ...
                    <input type="search" name="q"
                        ...
                        placeholder="Buscar algo..." aria-label="Buscar" autocomplete="off">
                </form>

                <div class="topbar-right">
                    <div class="notification-center">
                        <button class="icon-button notification-toggle" type="button" title="Notificações"
                            ...
                            <?php if (!empty($smartNotifications)): ?>
                                <div class="notification-dropdown-list">
                                    <?php foreach ($smartNotifications as $notification): ?>
                                        <a href="<?= e($notification['action']) ?>" class="notification-dropdown-item"
                                            data-smart-notification data-notification-key="<?= e($notification['key']) ?>"
        ...
        <nav class="mobile-bottom-nav" aria-label="Navegação principal no celular">
            <a href="<?= url('dashboard') ?>"
                class="mobile-bottom-item <?= ($active ?? '') === 'dashboard' ? 'active' : '' ?>"
            ...
            <a href="<?= url('history') ?>"
                class="mobile-bottom-item <?= ($active ?? '') === 'history' ? 'active' : '' ?>"
                data-mobile-nav="history">
                <i data-lucide="notebook-tabs"></i><span>Histórico</span>
            </a>
            <a href="<?= url('calendar') ?>"
                class="mobile-bottom-item <?= ($active ?? '') === 'calendar' ? 'active' : '' ?>"
                data-mobile-nav="calendar">
                <i data-lucide="calendar-days"></i><span>Calendário</span>
            </a>
            <button type="button"
                class="mobile-bottom-item mobile-more-toggle <?= in_array(($active ?? ''), ['absences', 'reports', 'profile', 'settings', 'search'], true) ? 'active' : '' ?>"
                id="mobile-more-toggle" aria-label="Mais opções" aria-expanded="false">
                <i data-lucide="ellipsis"></i><span>Mais</span>
            </button>
        </nav>

        <div class="mobile-more-layer" id="mobile-more-layer" aria-hidden="true">
            <button class="mobile-more-backdrop" id="mobile-more-backdrop" type="button"
                aria-label="Fechar mais opções"></button>
            ...
            <section class="mobile-more-sheet" role="dialog" aria-modal="true" aria-labelledby="mobile-more-title">
                ...
                <div class="mobile-more-grid">
                    <a href="<?= url('absences') ?>" class="<?= ($active ?? '') === 'absences' ? 'active' : '' ?>">
                        <span><i data-lucide="calendar-off"></i></span><strong>Ausências</strong><small>Justificativas,
                            folgas e férias</small>
                    </a>
                    <a href="<?= url('reports') ?>" class="<?= ($active ?? '') === 'reports' ? 'active' : '' ?>">
                        <span><i
                                data-lucide="chart-no-axes-column-increasing"></i></span><strong>Relatórios</strong><small>Horas,
                            banco e valores</small>
                    </a>
                    <a href="<?= url('profile') ?>" class="<?= ($active ?? '') === 'profile' ? 'active' : '' ?>">
                        <span><i data-lucide="user-round"></i></span><strong>Perfil</strong><small>Dados pessoais e
                            foto</small>
                    </a>
                    <a href="<?= url('settings') ?>" class="<?= ($active ?? '') === 'settings' ? 'active' : '' ?>">
                        <span><i data-lucide="settings"></i></span><strong>Configurações</strong><small>Jornada, tema e
                            notificações</small>
                    </a>
                    <a href="<?= url('search') ?>" class="<?= ($active ?? '') === 'search' ? 'active' : '' ?>">
                        <span><i data-lucide="search"></i></span><strong>Busca</strong><small>Encontre telas e
                            registros</small>
                    </a>
                    <a href="<?= url('logout') ?>" class="danger">
                        <span><i data-lucide="log-out"></i></span><strong>Sair</strong><small>Encerrar sua
                            sessão</small>
                    </a>
                </div>
            </section>
